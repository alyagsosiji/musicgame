...
const horizonNotices = [
    ...
    },
    {
        date: "2026-05-22",
        title: "🔔 게임 플레이 시 참고사항",
        content: "본 게임은 웹을 기반으로한 인디 게임으로, 다른 탭(사이트를) 많이 열어두거나, 브라우저 설정 중 GPU 하드웨어 가속 설정이 OFF 로 되어있다면, 게임 플레이에 부정적 영햐잉 있을 수 있습니다."
    },
    {
        date: "2026-05-19",
...
const charts = {
    easy: [
        ...
        {time: 13.5, lane: 1}, {time: 15.0, lane: 2}, {time: 16.5, lane: 0}, {time: 18.0, lane: 1},
        {time: 19.5, lane: 2}, {time: 21.0, lane: 3}, {time: 22.0, lane: 2}, {time: 23.5, lane: 1},
        {time: 25.0, lane: 0}, {time: 26.5, lane: 3}, {time: 28.0, lane: 1}, {time: 29.5, lane: 2},
        {time: 31.0, lane: 0}, {time: 32.5, lane: 3}, {time: 34.0, lane: 1}, {time: 35.5, lane: 2},
        {time: 37.0, lane: 0}, {time: 38.5, lane: 1}, {time: 40.0, lane: 2}, {time: 41.5, lane: 3},
        {time: 43.0, lane: 1}, {time: 44.5, lane: 2}, {time: 45.5, lane: 0}, {time: 46.5, lane: 3},
        {time: 47.5, lane: 1}, {time: 48.5, lane: 2}, {time: 49.5, lane: 0}, {time: 50.5, lane: 1},
        {time: 51.5, lane: 2}, {time: 52.5, lane: 3}, {time: 53.5, lane: 0}, {time: 54.0, lane: 1},
        {time: 54.5, lane: 2}, {time: 55.0, lane: 3}, {time: 56.0, lane: 0}, {time: 57.0, lane: 2},
        {time: 58.0, lane: 1}, {time: 59.0, lane: 3}, {time: 60.0, lane: 2}, {time: 61.0, lane: 0},
        {time: 62.0, lane: 3}, {time: 63.0, lane: 1}, {time: 64.0, lane: 0}, {time: 65.0, lane: 2},
        {time: 66.0, lane: 1}, {time: 67.0, lane: 3}, {time: 68.0, lane: 2}, {time: 69.0, lane: 1},
        {time: 70.0, lane: 0}, {time: 71.5, lane: 3}, {time: 73.0, lane: 1}, {time: 74.5, lane: 2},
        {time: 76.0, lane: 0}, {time: 77.5, lane: 3}, {time: 79.0, lane: 1}, {time: 80.5, lane: 2},
        {time: 82.0, lane: 0}, {time: 83.5, lane: 3}, {time: 85.0, lane: 2}, {time: 87.0, lane: 1},
        {time: 89.0, lane: 0}, {time: 91.0, lane: 3}, {time: 93.0, lane: 1}, {time: 95.0, lane: 2},
        {time: 97.0, lane: 0}, {time: 99.0, lane: 3}
    ],
    normal: [
        ...
        {time: 84.4, lane: 0}, {time: 84.4, lane: 3}, {time: 86.0, lane: 1}, {time: 87.0, lane: 2},
        {time: 88.0, lane: 0}, {time: 89.0, lane: 3}, {time: 90.5, lane: 1}, {time: 91.5, lane: 2},
        {time: 92.5, lane: 0}, {time: 93.5, lane: 3}, {time: 95.0, lane: 1}, {time: 96.5, lane: 2},
        {time: 98.0, lane: 0}, {time: 99.5, lane: 3}
    ],
    hard: [],
    master: []
};
...
(function generatePerfectCharts() {
    ...
    for (let t = 16.0; t < 40.0; t += 0.4) {
        let lane = Math.floor(t * 7) % 4; charts.hard.push({ time: parseFloat(t.toFixed(2)), lane: lane });
        if (parseFloat((t % 1.6).toFixed(1)) === 0) charts.hard.push({ time: parseFloat(t.toFixed(2)), lane: (lane + 2) % 4 });
    }
    for (let t = 40.0; t < 52.0; t += 0.2) charts.hard.push({ time: parseFloat(t.toFixed(2)), lane: Math.floor(t * 11) % 4 });
    for (let t = 52.0; t < 84.0; t += 0.2) {
        let lane = Math.floor(t * 13) % 4; charts.hard.push({ time: parseFloat(t.toFixed(2)), lane: lane });
        if (parseFloat((t % 0.8).toFixed(1)) === 0) charts.hard.push({ time: parseFloat(t.toFixed(2)), lane: (lane + 2) % 4 });
    }
    for (let t = 84.0; t < 100.0; t += 0.6) charts.hard.push({ time: parseFloat(t.toFixed(2)), lane: Math.floor(t * 5) % 4 });

    for (let t = 1.0; t < 16.0; t += 0.4) charts.master.push({ time: parseFloat(t.toFixed(2)), lane: Math.floor(t * 5) % 4 });
    for (let t = 16.0; t < 40.0; t += 0.2) {
        let lane = Math.floor(t * 13) % 4; charts.master.push({ time: parseFloat(t.toFixed(2)), lane: lane });
        if (parseFloat((t % 0.8).toFixed(1)) === 0) charts.master.push({ time: parseFloat(t.toFixed(2)), lane: (lane + 2) % 4 });
    }
    for (let t = 40.0; t < 52.0; t += 0.1) charts.master.push({ time: parseFloat(t.toFixed(2)), lane: Math.floor(t * 23) % 4 });
    for (let t = 52.0; t < 84.0; t += 0.1) {
        let lane = Math.floor(t * 29) % 4; charts.master.push({ time: parseFloat(t.toFixed(2)), lane: lane });
    ...
    for (let t = 84.0; t < 100.0; t += 0.3) charts.master.push({ time: parseFloat(t.toFixed(2)), lane: Math.floor(t * 7) % 4 });
...
function showCustomAlert(title, message, isConfirm = false, callback = null) {
    ...
    if(isConfirm) cancelBtn.classList.remove("hidden"); else cancelBtn.classList.add("hidden");
...
async function handleAuth() {
    ...
    if (isSignUpMode && rawPw.length < 6) return showCustomAlert("경고", "비밀번호는 최소 6자리 이상이어야 합니다.");
    ...
    if (inputIdHash === ADMIN_ID_HASH && inputPwHash === ADMIN_PW_HASH) {
        isAdmin = true; currentUser = { uid: "admin_asi", displayName: "아시" };
        document.getElementById("btn-admin").classList.remove("hidden");
        isAuthActionLock = false; showLobby(rawId); return;
    }
    ...
    rankingUnsubscribe = db.collection("horizon_rankings").orderBy("score", "desc").limit(20)
        .onSnapshot((snapshot) => {
            ...
            snapshot.forEach(doc => {
                const data = doc.data();
                tbody.innerHTML += `<tr><td>${rank++}</td><td>${data.username}</td><td>${data.difficulty.toUpperCase()}</td><td>${data.score}</td><td>${data.maxCombo}</td><td>${data.perfectCount}</td><td><span class="platform-badge">${data.platform}</span></td></tr>`;
            });
        }, e => console.error(e));
}
...
function requestDeleteRank(id) { showCustomAlert("확인", "해당 기록을 삭제하시겠습니까?", true, async () => { await db.collection("horizon_rankings").doc(id).delete(); }); }
function requestBanUser(id) {
    showCustomAlert("영구 추방", "해당 유저 및 등록된 모든 글로벌 랭킹 기록을 파기하시겠습니까?", true, async () => {
        ...
        if (userDoc.exists) {
            ...
            const batch = db.batch(); rankingSnap.forEach(doc => batch.delete(doc.ref)); await batch.commit(); 
        }
...
function startGame(diff) {
    ...
    score = 0; combo = 0; maxCombo = 0; perfectCount = 0; greatCount = 0; goodCount = 0; missCount = 0;
    hitParticles = []; hitRings = []; activeNotes = []; isPaused = false;
...
function gameLoop() {
    ...
    for(let l=0; l<4; l++) { if(lanePressed[l]) { let grad = ctx.createLinearGradient(lanes[l]-50, 0, lanes[l]+50, canvas.height); grad.addColorStop(0, "rgba(138, 43, 226, 0.0)"); grad.addColorStop(0.85, "rgba(138, 43, 226, 0.25)"); grad.addColorStop(1, "rgba(0, 255, 255, 0.1)"); ctx.fillStyle = grad; ctx.fillRect(lanes[l]-46, 0, 92, canvas.height); } }
    ctx.strokeStyle = "rgba(138, 43, 226, 0.3)"; lanes.forEach(x => { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, canvas.height); ctx.stroke(); });
    ctx.strokeStyle = "#00ffff"; ctx.lineWidth = 4; ctx.beginPath(); ctx.moveTo(0, targetY); ctx.lineTo(canvas.width, targetY); ctx.stroke();

    while (chartData.length > 0 && chartData[0].time <= syncTime + lookAheadTime) { const next = chartData.shift(); activeNotes.push({ targetTime: next.time, lane: next.lane, x: lanes[next.lane] }); }
    for (let i = activeNotes.length - 1; i >= 0; i--) {
        let n = activeNotes[i]; n.y = targetY - (n.targetTime - syncTime) * currentSpeedFactor;
        ctx.fillStyle = cachedNoteGradients[n.lane] || "#ff00ff"; ctx.fillRect(n.x - 44, n.y - 9, 88, 18);
        if (syncTime > n.targetTime + 0.15) { activeNotes.splice(i, 1); missCount++; updateJudgement("MISS"); }
    }
    for (let r = hitRings.length - 1; r >= 0; r--) {
        let ring = hitRings[r]; ring.radius += 2.5; ring.alpha -= 0.05; if (ring.alpha <= 0) { hitRings.splice(r, 1); continue; }
        ctx.strokeStyle = ring.color; ctx.globalAlpha = ring.alpha; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(ring.x, ring.y, ring.radius, 0, Math.PI * 2); ctx.stroke(); ctx.globalAlpha = 1.0;
    }
    for (let p = hitParticles.length - 1; p >= 0; p--) {
        let pt = hitParticles[p]; pt.x += pt.vx; pt.y += pt.vy; pt.alpha -= 0.04; if(pt.alpha <= 0) { hitParticles.splice(p, 1); continue; }
        ctx.fillStyle = (pt.color || `rgba(0, 255, 255, `) + pt.alpha + ")"; ctx.beginPath(); ctx.arc(pt.x, pt.y, pt.size, 0, Math.PI * 2); ctx.fill();
    }
    if (audio && audio.ended) { finishGame(); return; }
    if (!isPaused) animationId = requestAnimationFrame(gameLoop);
}

function createSparks(startX, startY, judgment) {
    let count = 15, sizeMax = 3, ringColor = "#00ffff", colorPrefix = "rgba(0, 255, 255, ";
    if (judgment === "PERFECT") { count = 26; colorPrefix = "rgba(255, 215, 0, "; sizeMax = 4.2; ringColor = "#ffd700"; }
    else if (judgment === "GREAT") { count = 18; colorPrefix = "rgba(255, 0, 255, "; sizeMax = 3.5; ringColor = "#ff00ff"; }
    else if (judgment === "GOOD") { count = 10; colorPrefix = "rgba(0, 220, 255, "; sizeMax = 2.5; ringColor = "#00dcff"; }

    for(let i=0; i<count; i++) hitParticles.push({ x: startX, y: startY, vx: (Math.random() - 0.5) * 9, vy: (Math.random() - 0.7) * 9, size: Math.random() * sizeMax + 1, alpha: 1.0, color: colorPrefix });
    hitRings.push({ x: startX, y: startY, radius: 4, maxRadius: 45, alpha: 1.0, color: ringColor });
}

function verifyHit(lane) {
    if(!gameActive || isPaused) return; let syncTime = currentAudioTime + audioOffset;
    for(let i=0; i<activeNotes.length; i++) {
        let n = activeNotes[i];
        if(n.lane === lane) {
            let timeDiff = Math.abs(n.targetTime - syncTime);
            if(timeDiff < 0.15) {
                let judgeStr = "GOOD"; if(timeDiff < 0.05) { judgeStr = "PERFECT"; score += 1000; perfectCount++; } else if(timeDiff < 0.10) { judgeStr = "GREAT"; score += 500; greatCount++; } else { score += 200; goodCount++; }
                updateJudgement(judgeStr); createSparks(n.x, targetY, judgeStr); activeNotes.splice(i, 1); break;
            } break;
        }
    }
}
...
function updateJudgement(res) {
    ...
    if(res === "MISS") combo = 0; else { combo++; if(combo > maxCombo) maxCombo = combo; }
...
window.addEventListener("keydown", e => {
    ...
    if (e.key === "ArrowLeft") { adjustAudioOffset(-0.01); return; } if (e.key === "ArrowRight") { adjustAudioOffset(0.01); return; }
    if (k === "p") { togglePauseGame(); return; }
    if (keyMap[k] !== undefined) { lanePressed[keyMap[k]] = true; verifyHit(keyMap[k]); }
});
